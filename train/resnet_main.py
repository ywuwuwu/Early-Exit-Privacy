...
from branchyResnet import branchy_ResNet18
from branchyResnet import branchy_ResNet18_inference
...
model_branchy_resnet = branchy_ResNet18(args)
model_branchy_resnet_inference = branchy_ResNet18_inference(args)
input_shape = (None, 32, 32, 3)
model_branchy_resnet.build(input_shape)
model_branchy_resnet_inference.build(input_shape)
callback = PrivacyMetrics(epochs_per_report, "branchy ResNet18", model_branchy_resnet_inference)

callback = training_loop(x_train, y_train, x_test,y_test,callback, model_branchy_resnet,model_branchy_resnet_inference,optimizer, metric, metric_test, batch_size = batch_size, total_epochs = total_epochs, lr = lr)
...
epoch_plot.savefig('results/branchy/resnet_epoch_plot.pdf')
...
utility_privacy_plot.savefig('results/figures/resnet_utility_privacy.pdf')
